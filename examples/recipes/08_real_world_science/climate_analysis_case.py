...
class AnalysisCheckpointer:
    ...
    def load_progress(self):
        ...
        if self.progress_file.exists():
            ...
            self.progress = {"steps_completed": [], "last_updated": None, "data_info": {}}
...
def create_synthetic_climate_data(
    output_dir: Path, start_year: int = 2000, end_year: int = 2020, spatial_resolution: float = 1.0
) -> List[Path]:
    ...
    with timer(f"Creating {end_year - start_year} years of data") as t:
        for year in range(start_year, end_year):
            ...
            for i, (day, seasonal_temp) in enumerate(zip(day_of_year, seasonal_cycle)):
                ...
                temp_data[i] = (
                    base_temp + seasonal_temp + topo_var + trend + daily_noise + 273.15
                )  # Convert to Kelvin

                # Precipitation with seasonal and spatial patterns
                # More rain in tropics and during summer
                seasonal_precip = np.maximum(
                    0, 5 * (1 - np.abs(lat_mesh / 30)) * (1 + np.cos(2 * np.pi * day / 365.25))
                )
...
def load_and_preprocess_data(
    file_paths: List[Path], checkpointer: AnalysisCheckpointer, client
) -> xr.Dataset:
    ...
    with timer("Data loading and preprocessing") as t:
        ...
        ds["temperature_celsius"].attrs.update(
            {"units": "degrees_C", "long_name": "Near-surface air temperature in Celsius"}
        )

        # Monthly precipitation totals
        ds["precipitation_monthly"] = ds["precipitation"].resample(time="1M").sum()
        ds["precipitation_monthly"].attrs.update(
            {"units": "mm/month", "long_name": "Monthly precipitation total"}
        )
        ...
        if ZARR_AVAILABLE:
            ...
            encoding = {
                var: {"compressor": zarr.Blosc(cname="lz4", clevel=5)} for var in ds.data_vars
            }
...
def compute_climatologies(
    ds: xr.Dataset, checkpointer: AnalysisCheckpointer, client
) -> Dict[str, xr.Dataset]:
    ...
    for clim_type, clim_data in climatologies.items():
        ...
        print(
            f"   {clim_type.capitalize()} precipitation: {precip_mean:.1f}±{precip_std:.1f} mm/day"
        )
...
def analyze_temperature_trends(
    ...
    with timer("Temperature trend analysis") as t:
        ...
        results["global_trend"] = {
            ...
            "p_value": p_value,
            "std_error": std_err,
            "time_series": {"years": years.tolist(), "temperatures": temps.tolist()},
        }
        ...
        for region_name, bounds in regions.items():
            ...
            r_slope, r_intercept, r_r_value, r_p_value, r_std_err = linregress(
                years, regional_temps
            )
        ...
        print(
            f"   Significant warming: {significant_warming}/{total_points} pixels ({100 * significant_warming / total_points:.1f}%)"
        )
        print(
            f"   Significant cooling: {significant_cooling}/{total_points} pixels ({100 * significant_cooling / total_points:.1f}%)"
        )
...
def regrid_to_standard_grid(
    ds: xr.Dataset, target_resolution: float, checkpointer: AnalysisCheckpointer, client
) -> xr.Dataset:
    ...
    with timer("Data regridding") as t:
        ...
        regridded = annual_data.interp(lat=target_lats, lon=target_lons, method="linear")
        ...
        # Save checkpoint
        if ZARR_AVAILABLE:
            checkpoint_file = checkpointer.get_checkpoint_path(
                f"regridded_{target_resolution}deg.zarr"
            )
...
def create_analysis_plots(
    ...
    with timer("Plot generation") as t:
        ...
        if "global_trend" in trend_results:
            ...
            ax.plot(
                years,
                slope * np.array(years) + trend_results["global_trend"]["intercept"],
                "r--",
                linewidth=2,
        ...
        if "regional_trends" in trend_results:
            ...
            bars = ax.bar(
                range(len(regions)), trends, yerr=errors, capsize=5, alpha=0.7, color="skyblue"
            )
        ...
        if "seasonal" in climatologies:
            ...
            for i, (season, season_name) in enumerate(zip(seasons, season_names)):
                if season in seasonal.season:
                    ...
                    im = axes[i].contourf(
                        temp_data.lon, temp_data.lat, temp_data, levels=20, cmap="RdYlBu_r"
                    )
...
def create_analysis_summary(checkpointer: AnalysisCheckpointer, output_dir: Path) -> Path:
    ...
    with open(summary_file, "w") as f:
        ...
        if "climatologies" in progress["data_info"]:
            ...
            for clim_type, stats in info["precipitation_stats"].items():
                f.write(
                    f"  - {clim_type.capitalize()}: {stats['mean']:.1f}±{stats['std']:.1f} mm/day\n"
                )
            f.write("\n")
        ...
        if "temperature_trends" in progress["data_info"]:
            ...
            f.write(
                f"- **Spatial analysis:** {spatial['significant_warming_pixels']}/{spatial['total_pixels']} pixels "
            )
            f.write(
                f"showing significant warming ({100 * spatial['significant_warming_pixels'] / spatial['total_pixels']:.1f}%)\n\n"
            )
        ...
        for step_name, step_info in progress["data_info"].items():
            if step_name.startswith("regridding_"):
                ...
                f.write(
                    f"- **Original resolution:** {step_info['original_shape'][0]}×{step_info['original_shape'][1]}\n"
                )
                f.write(
                    f"- **Target resolution:** {step_info['target_shape'][0]}×{step_info['target_shape'][1]} "
                )
...
def main():
    ...
    parser.add_argument(
        "--start-year", type=int, default=2000, help="Start year for analysis (default: 2000)"
    )

    parser.add_argument(
        "--end-year", type=int, default=2010, help="End year for analysis (default: 2010)"
    )
    ...
    try:
        ...
        regridded_ds = regrid_to_standard_grid(ds, args.regrid_resolution, checkpointer, client)
        ...
        if ZARR_AVAILABLE:
            ...
            total_zarr_size = sum(
                sum(f.stat().st_size for f in zf.rglob("*") if f.is_file()) for zf in zarr_files
            ) / (1024**3)
