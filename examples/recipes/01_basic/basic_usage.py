    ...
    if hasattr(cluster, 'dashboard_link') and cluster.dashboard_link:
        ...
        try:
            ...
            if any(hpc in hostname.lower() for hpc in ['gadi', 'raijin', 'magnus', 'pawsey']):
                ...
                print(f"🔗 SSH tunnel (for HPC): ssh -N -L 8787:{hostname}:{port} <your_login_node>")
        except Exception:
            pass
...
def run_cpu_demo(max_workers: int = None, reserve_mem_gb: float = 50.0, verbose: bool = False) -> tuple:
...
def run_io_demo(max_workers: int = None, reserve_mem_gb: float = 50.0, verbose: bool = False) -> tuple:
...
def run_mixed_demo(max_workers: int = None, reserve_mem_gb: float = 50.0, verbose: bool = False) -> tuple:
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Maximum number of workers (default: auto-detect)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=50.0,
        help="Memory to reserve for OS in GB (default: 50.0)"
    )
    ...
    try:
        ...
        if args.workload in ["io", "all"]:
            io_client, io_cluster, _, io_time = run_io_demo(
                args.workers, args.memory, args.verbose
            )
            if io_client:
