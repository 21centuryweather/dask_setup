...
def create_benchmark_dataset(
    size_gb: float, chunk_size_mb: float = 256, dimensions: Tuple[str, ...] = ("time", "lat", "lon")
) -> da.Array:
...
def analyze_chunk_size_optimization(
    worker_count: int, data_size_gb: float, chunk_sizes_mb: List[float], workload_type: str = "cpu"
) -> List[BenchmarkResult]:
...
def create_performance_plots(
    ...
    if scaling_results:
        ...
        for workload_type, results in scaling_results.items():
            ...
            ax2.plot(
                workers, efficiencies, "s-", label=f"{workload_type}", linewidth=2, markersize=6
            )
    ...
    if scaling_results:
        ...
        for workload_type, results in scaling_results.items():
            ...
            ax.plot(workers, throughputs, "o-", label=f"{workload_type}", linewidth=2, markersize=8)
    ...
    for result in chunk_results:
        chunk_data.append(
            [
                ...
                result.notes.split("chunks=")[1].split(",")[0]
                if "chunks=" in result.notes
                else "N/A",
                f"{result.computation_time:.2f}",
                f"{result.throughput_gb_s:.2f}",
            ]
    ...
    with open(output_file, "w") as f:
        ...
        if best_scaling:
            ...
            for workload_type, result in best_scaling.items():
                f.write(
                    f"- {workload_type.upper()}: Best performance with {result.workers} workers "
                )
                f.write(f"({result.throughput_gb_s:.2f} GB/s throughput)\n")

        if optimal_chunk:
            f.write(
                f"**Chunk Optimization:** Optimal chunk size is {optimal_chunk.chunk_size_mb:.0f} MB "
            )
        ...
        if adaptive_results:
            ...
            if history:
                ...
                f.write(
                    f"**Adaptive Scaling:** Converged to {adaptive_results['final_workers']} workers "
                )
        ...
        if scaling_data:
            ...
            f.write(
                "| Workload | Workers | Time (s) | Throughput (GB/s) | Scaling Eff. | Memory (GB) |\n"
            )
            f.write(
                "|----------|---------|----------|-------------------|--------------|-------------|\n"
            )
        ...
        f.write(
            "**Strong Scaling:** Measures how performance improves with more workers for fixed problem size.\n"
        )
        ...
        for workload_type, results in scaling_results.items():
            ...
            if efficient_results:
                ...
                f.write(
                    f"- Recommended workers: {optimal_workers} (good efficiency + high throughput)\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--max-workers", type=int, default=8, help="Maximum number of workers to test (default: 8)"
    )
    ...
    parser.add_argument("--skip-chunking", action="store_true", help="Skip chunk optimization")

    parser.add_argument(
        "--skip-adaptive", action="store_true", help="Skip adaptive scaling demonstration"
    )
    ...
    try:
        ...
        plot_files = create_performance_plots(scaling_results, chunk_results, args.output_dir)

        # Create optimization report
        report_file = args.output_dir / "performance_optimization_report.md"
        create_optimization_report(scaling_results, chunk_results, adaptive_results, report_file)
        ...
        # Summary statistics
        total_benchmarks = sum(len(results) for results in scaling_results.values()) + len(
            chunk_results
        )
