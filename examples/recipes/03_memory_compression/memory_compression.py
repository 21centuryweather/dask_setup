...
try:
    ...
    from utils import (
        format_duration, get_worker_memory_stats, pretty_bytes, 
        save_markdown_table, timer
    )
...
def get_spill_stats(client) -> Dict[str, any]:
    ...
    try:
        ...
        if worker_stats:
            ...
            avg_percent = sum(stats.get('memory_percent', 0) for stats in worker_stats.values()) / len(worker_stats)
            
            # Aggregate disk usage
            total_tmpdir_used = sum(stats.get('tmpdir_used_gb', 0) for stats in worker_stats.values())
            total_tmpdir_free = sum(stats.get('tmpdir_free_gb', 0) for stats in worker_stats.values())
...
def monitor_memory_during_computation(client, computation_func, description: str = "Computation") -> Tuple[any, List[Dict]]:
...
def analyze_benchmark_results(results: Dict[str, Dict]) -> None:
    ...
    for compression, data in successful_results.items():
        ...
        if 'total_memory_rss_mb' in spill_stats:
            ...
            print(f"   {compression:10}: RSS={rss_mb:6.1f}MB, "
...
def save_benchmark_summary(results: Dict[str, Dict], output_file: Path) -> None:
    ...
    table_data = []
    headers = ["Compression", "Time (s)", "Relative Performance", "Memory (MB)", "Spill (GB)", "Status"]
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Size of test arrays in GB (default: 1.0)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Maximum number of workers (default: 2)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=20.0,
        help="Memory to reserve for OS in GB (default: 20.0)"
    )
