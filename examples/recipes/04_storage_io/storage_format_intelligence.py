...
def demonstrate_filesystem_caching(
    file_paths: List[Path],
    client = None
) -> Dict[str, float]:
...
def create_performance_summary_table(
    ...
    if comparison_results:
        ...
        for format_name, results in comparison_results.items():
            if format_name in ['netcdf', 'zarr'] and results.get('success'):
                relative_speed = "1.0x (baseline)" if format_name == 'netcdf' else f"{comparison_results.get('speedup', 1.0):.2f}x"
                format_data.append([
                    format_name.upper(),
    ...
    if caching_results:
        ...
        cache_data = [
            ...
            ["Warm Cache", f"{caching_results.get('warm_cache', 0):.2f}", f"{caching_results.get('cache_speedup', 1.0):.2f}x"]
        ]
    ...
    for engine, results in engine_results.items():
        if results['success']:
            all_data.append(["NetCDF Engine", f"{engine} multi-file", f"{results['multi_file_time']:.2f}s", "Parallel reading"])
    
    # Add format comparison
    for format_name, results in comparison_results.items():
        if format_name in ['netcdf', 'zarr'] and results.get('success'):
            all_data.append(["Format", format_name.upper(), f"{results['total_time']:.2f}s", "Complete workflow"])
    
    # Add conversion info
    if conversion_results.get('success'):
        all_data.append(["Conversion", "NetCDF → Zarr", f"{conversion_results['conversion_time']:.2f}s", f"{conversion_results['zarr_size_mb']:.1f}MB output"])
    
    # Add caching
    if 'cache_speedup' in caching_results:
        all_data.append(["Caching", "Speedup", f"{caching_results['cache_speedup']:.2f}x", "Warm vs cold cache"])
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of NetCDF files to create (default: 20)"
    )
    ...
    parser.add_argument(
        ...
        nargs='+',
        default=None,
        help="NetCDF engines to test (default: all available)"
    )
    ...
    try:
        ...
        create_performance_summary_table(
            engine_results, conversion_results, comparison_results,
            caching_results, summary_file
        )
        ...
        if zarr_store.exists():
            zarr_size = sum(f.stat().st_size for f in zarr_store.rglob('*') if f.is_file()) / (1024**2)
