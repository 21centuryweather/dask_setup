...
class RechunkPipeline:
    ...
    def _recommend_chunks(self, ds: xr.Dataset, analysis: Dict) -> Dict[str, Any]:
        ...
        spatial_dims = [d for d in dims.keys() if d in ['lat', 'lon', 'latitude', 'longitude', 'x', 'y']]
    ...
    def execute_rechunk_plan(
        ...
        try:
            ...
            temp_peak_size = self._calculate_zarr_size(temp_store) / (1024**3) if temp_store.exists() else 0.0
        ...
        try:
            ...
            for var_name in target_ds.data_vars:
                if var_name in zarr_store:
                    ...
                    validation['compression_info'][var_name] = str(compressor) if compressor else 'none'
                ...
                try:
                    ...
                    for dim_name, dim_size in zip(source_ds[var_name].dims, var_shape):
                        ...
                        if sample_size < dim_size:
                            # Random sample
                            indices = np.sort(np.random.choice(dim_size, sample_size, replace=False))
...
def create_integration_report(
    ...
    successful_workflows = [r for r in workflow_results if r.get('rechunk_result', {}).get('success', False)]
    total_workflows = len(workflow_results)
    
    # Calculate summary stats
    total_data_processed = sum(
        r.get('rechunk_result', {}).get('source_size_gb', 0) 
    ...
    avg_compression = np.mean([
        ...
        for r in successful_workflows
    ]) if successful_workflows else 1.0
    
    avg_time = np.mean([
        r.get('rechunk_result', {}).get('rechunk_time', 0)
        for r in successful_workflows  
    ]) if successful_workflows else 0.0
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"- **Successful conversions:** {len(successful_workflows)} ({len(successful_workflows)/total_workflows*100:.1f}%)\n")
...
def main():
    ...
    parser.add_argument(
        "--source-file",
        type=Path,
        help="Source NetCDF file (will create sample if not provided)"
    )
    
    parser.add_argument(
        "--create-sample",
        action="store_true",
        help="Create sample NetCDF file for testing"
    )
    ...
    parser.add_argument(
        "--max-mem",
        default="4GB",
        help="Maximum memory per worker for rechunking (default: 4GB)"
    )
    ...
    parser.add_argument(
        "--temp-dir",
        type=Path,
        help="Temporary storage directory (default: output_dir/temp)"
    )
    ...
    parser.add_argument(
        "--keep-temp",
        action="store_true", 
        help="Keep temporary storage (don't cleanup)"
    )
    ...
    try:
        ...
        if args.source_file and args.source_file.exists():
            ...
            if not source_file.exists() or args.create_sample:
                source_file = create_sample_netcdf_dataset(
                    source_file, time_steps, spatial_shape
                )
            else:
        ...
        if target_zarr.exists():
            zarr_size = sum(f.stat().st_size for f in target_zarr.rglob('*') if f.is_file()) / (1024**3)
