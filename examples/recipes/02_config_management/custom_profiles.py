...
def demonstrate_builtin_profiles(verbose: bool = False) -> Dict[str, float]:
    ...
    for name, profile in builtin_profiles.items():
        ...
        if verbose:
            print(f"    Workload: {profile.config.workload_type}, "
...
def demonstrate_inline_config_override(verbose: bool = False) -> Optional[float]:
    ...
    try:
        ...
        with dask.config.set({
            ...
            print(f"   scheduler.idle-timeout: {dask.config.get('distributed.scheduler.idle-timeout')}")
            print(f"   worker.daemon: {dask.config.get('distributed.worker.daemon')}")
            print(f"   worker.memory.rebalance.measure: {dask.config.get('distributed.worker.memory.rebalance.measure')}")
...
def load_and_test_yaml_profile(profile_path: Path, verbose: bool = False) -> Optional[float]:
    ...
    try:
        ...
        config_data = {k: v for k, v in profile_data.items() 
...
def print_performance_summary(results: Dict[str, Optional[float]]) -> None:
    ...
    valid_results = {name: time for name, time in results.items() 
...
def main():
    ...
    try:
        ...
        if args.demo in ["yaml", "all"]:
            profile_path = args.profile or (Path(__file__).parent.parent / "configs" / "cpu_profile.yaml")
