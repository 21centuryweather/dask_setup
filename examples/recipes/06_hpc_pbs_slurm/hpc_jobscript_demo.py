...
def detect_hpc_environment() -> Dict[str, any]:
    ...
    if any(pbs_vars.values()):
        ...
        env_info['recommendations'].append(
            "Not running under PBS or SLURM scheduler"
        )
...
def demonstrate_job_monitoring() -> None:
    ...
    if pbs_job:
        ...
        try:
            import subprocess
            result = subprocess.run(['qstat', '-f', pbs_job], 
...
def main():
    ...
    parser.add_argument(
        "--generate-scripts",
        action="store_true",
        help="Generate example PBS and SLURM job scripts"
    )
    ...
    parser.add_argument(
        "--walltime",
        default="4:00:00",
        help="Wall time limit (default: 4:00:00)"
    )
    
    parser.add_argument(
        "--test-cluster",
        action="store_true",
        help="Test cluster creation and dashboard access"
    )
