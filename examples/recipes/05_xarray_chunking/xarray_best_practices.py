...
def analyze_chunking_options(
    ds: xr.Dataset, chunk_options: Dict[str, Dict[str, int]] = None
) -> Dict[str, Dict]:
    ...
    for option_name, chunks in chunk_options.items():
        ...
        try:
            ...
            results[option_name] = {"error": str(e), "quality": "❌ Failed"}

    return results


def benchmark_chunking_performance(
    ds: xr.Dataset, chunk_options: Dict[str, Dict[str, int]], operation: str = "mean", client=None
) -> Dict[str, float]:
...
def create_chunking_comparison_table(
    ...
    print("\nComparison Table:")
    col_widths = [
        max(len(str(row[i])) for row in [headers] + table_data) for i in range(len(headers))
    ]
...
def main():
    ...
    try:
        ...
        client, cluster, temp_dir = setup_dask_client(
            workload_type=args.workload, max_workers=args.workers, dashboard=True
        )
        ...
        create_chunking_comparison_table(analysis_results, performance_results, output_file)
        ...
        if RECHUNKER_AVAILABLE and valid_options:
            ...
            if source_chunks and target_chunks:
                rechunk_time = demonstrate_rechunking(
                    ds, source_chunks, target_chunks, temp_dir + "/rechunker_demo"
                )
