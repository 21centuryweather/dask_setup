...
def demonstrate_file_access_errors() -> Dict[str, Any]:
    ...
    with capture_error_context("Permission Issues", expected_error=False) as perm_demo:
        ...
        try:
            ...
            restricted_path = (
                "/root/test_file.nc" if os.name != "nt" else "C:\\Windows\\test_file.nc"
            )
    ...
    with capture_error_context("Large Chunks", expected_error=True) as large_chunks:
        ...
        if worker_info:
            ...
            if worker_memory:
                ...
                large_chunk_array = da.random.random(
                    (chunk_dim * 2, chunk_dim * 2), chunks=(chunk_dim, chunk_dim)
                )
...
def demonstrate_performance_bottlenecks(client) -> Dict[str, Any]:
    ...
    with capture_error_context("GIL Contention", expected_error=False) as gil_demo:
        ...
        result = (
            arr.map_blocks(lambda x: np.vectorize(cpu_bound_function)(x), dtype=arr.dtype)
            .sum()
            .compute()
        )
    ...
    with capture_error_context("I/O Bottleneck", expected_error=False) as io_demo:
        ...
        start_time = time.time()
        result = arr.map_blocks(simulate_io_delay, dtype=np.float64, drop_axis=[1]).sum().compute()
...
def create_troubleshooting_guide_table(
    ...
    troubleshooting_data = [
        ...
        [
            "Memory",
            "MemoryError/KilledWorker",
            "Reduce chunk size, increase workers",
            "Use chunks targeting 256-512MB",
        ],
        [
            "Memory",
            "Task exceeds memory limit",
            "Lower reserve_mem_gb parameter",
            "Check worker memory with client.scheduler_info()",
        ],
        [
            "Memory",
            "Spilling too aggressive",
            "Tune spill thresholds in config",
            "Monitor with dashboard memory tab",
        ],
        # Worker Issues
        [
            "Workers",
            ...
            "Check system resources with htop/top",
        ],
        [
            "Workers",
        ...
        [
            ...
            "DiskSpaceError",
            "Free up disk space",
            "Use $PBS_JOBFS or $TMPDIR for temporary files",
        ],
        # Chunking
        ...
        [
            "Chunking",
            "Uneven chunks",
            "Use explicit chunk sizes",
            "Avoid 'auto' chunking for consistent performance",
        ],
        # Performance
        [
            "Performance",
            ...
            "Set workload_type='cpu' in setup_dask_client",
        ],
        [
            "Performance",
            "I/O bottleneck",
            "Use more threads, fewer processes",
            "Set workload_type='io' in setup_dask_client",
        ],
        [
            "Performance",
            "Network overhead",
            "Optimize chunk size",
            "Reduce data transfer between workers",
        ],
        # Dashboard
        ...
        [
            ...
            "Export NCPUS, PBS_MEM etc. if needed",
        ],
        [
            "Config",
            "Temp directory issues",
            "Check $PBS_JOBFS access",
    ...
    successful_demos = sum(
        sum(1 for k, v in r.items() if k != "error" and isinstance(v, dict) and v is not None)
...
def main():
    ...
    parser.add_argument(
        "--workers", type=int, default=2, help="Number of Dask workers (default: 2)"
    )
    ...
    if PSUTIL_AVAILABLE:
        ...
        print(
            f"System memory: {memory_info['used']:.1f}/{memory_info['total']:.1f} GB "
