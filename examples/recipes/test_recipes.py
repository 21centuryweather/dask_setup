...
def test_imports() -> Tuple[bool, List[str]]:
    ...
    critical_imports = ["dask", "distributed", "xarray", "numpy", "pandas"]

    optional_imports = ["matplotlib", "scipy", "zarr", "rechunker", "psutil", "netcdf4", "h5netcdf"]
...
def test_dask_setup_basic() -> bool:
    ...
    try:
        ...
        client, cluster, temp_dir = dask_setup.setup_dask_client(
            workload_type="cpu", max_workers=1, reserve_mem_gb=2.0, dashboard=False
        )
...
def test_sample_recipe() -> bool:
    ...
    try:
        ...
        spec = importlib.util.spec_from_file_location("basic_usage", basic_dir / "basic_usage.py")
